...
export const createConfigFile = (options: Config) => {
  createFile("./yureka.config.json", JSON.stringify(options, null, 2));
};
...
export const readConfigFile = (): (Config & { rootPath: string }) | null => {
  ...
  const configPath = path.join(process.cwd(), "yureka.config.json");
...
export const sendEvent = async (
  ...
  const url = "https://yureka.vercel.app";
  try {
    await fetch(url + `/api/send-event`, {
      method: "POST",
      headers: {
        "x-request-from": "create yureka",
      },
      body: JSON.stringify({
        event,
        config,
