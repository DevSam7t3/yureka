...
yureka generated the following resources for \`${tableNameNormalEnglishSingular}\`:
${resources.map((r) => `- ${resourceMapping[r]}`).join("\n")}

${createNextStepsList(nextStepsList)}

${createNotesList([
  "If you run into any issues, please create an issue on GitHub\n  (https://github.com/nicoalbanese/yureka/issues)",
  "Documentation (https://yureka.dev/commands/generate)",
])}`);
};
