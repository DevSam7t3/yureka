...
export const printNextSteps = (
  ...
  const packagesInstalledList = [
    ...(promptResponses.orm === "drizzle"
      ? [
          `${pc.underline("ORM")}: Drizzle (using ${
            promptResponses.dbProvider
          })`,
        ]
      : []),
    ...(promptResponses.orm === "prisma"
      ? [`${pc.underline("ORM")}: Prisma`]
      : []),
    ...(promptResponses.auth === "next-auth"
      ? [
          `${pc.underline("Authentication")}: Auth.js ${
...
export const showNextSteps = (
  ...
  const nextStepsFormatted = `🚀 Thanks for using Yureka!
...
Hint: use \`yureka generate\` to quickly scaffold entire entities for your application`;
